    ...
    if (isset($payment_info['status']) && $payment_info['status'] === 'approved') {
        ...
        if ($turnoId > 0) {
            ...
            if (!$conn->connect_error) {
                ...
                if ($stmt->execute()) {
                    file_put_contents("/../logs/log_webhook.txt", "[OK] !!! ÉXITO !!! Turno $turnoId actualizado." . PHP_EOL, FILE_APPEND);
                }
                $stmt->close();
                $conn->close();
            }
        } else {
            file_put_contents("/../logs/log_webhook.txt", "[ALERTA] Pago $payment_id sin external_reference. Data: " . json_encode($payment_info['external_reference']) . PHP_EOL, FILE_APPEND);
        }
    } else {
