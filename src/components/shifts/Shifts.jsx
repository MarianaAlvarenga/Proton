...
const Shifts = () => {
  ...
  useEffect(() => {
    if (userRole === 1 || userRole === 4 || isAgendarTurno) {
      const fetchEspecialidades = async () => {
        try {
          const res = await axios.get('https://explained-branches-post-lease.trycloudflare.com/backend/actions/getEspecialidades.php');
  ...
  useEffect(() => {
    ...
    const fetchPeluqueros = async () => {
      try {
        const res = await axios.get(`https://explained-branches-post-lease.trycloudflare.com/backend/actions/getPeluquerosByServicio.php?id_servicio=${selectedEspecialidad}`);
