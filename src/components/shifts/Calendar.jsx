...
export default function Calendar({
  ...
  const fetchDisponibilidades = (idPeluquero) => {
    ...
    setLoading(true);
    fetch(
      `https://explained-branches-post-lease.trycloudflare.com/backend/actions/getAvailabilities.php?id_peluquero=${idPeluquero}`
    )
      .then((res) => {
        if (!res.ok) {
  ...
  const handleEventClick = async (info) => {
    ...
    if (isAgendarTurno) {
      ...
      try {
        const res = await fetch(
          "https://explained-branches-post-lease.trycloudflare.com/backend/actions/saveAppointment.php",
          {
            method: "POST",
    ...
    if (isSettingAvailability && userRole === 3) {
      ...
      try {
        const res = await fetch("https://explained-branches-post-lease.trycloudflare.com/backend/actions/deleteAvailability.php", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        "https://explained-branches-post-lease.trycloudflare.com/backend/actions/availability.php",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
