...
const Asistencia = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://academy-prices-stylish-nevada.trycloudflare.com/backend/actions/getTurnoById.php?id_turno=${turnoBase.id_turno}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setTurno(data.turno);
  ...
  const handleSubmit = () => {
    fetch("https://academy-prices-stylish-nevada.trycloudflare.com/backend/actions/saveAsistencia.php", {
      method: "POST",
