  ...
  const handleClick = async () => {
    if (NameButton === "Finalizar compra") {
      try {
        ...
        if (isRegistered) {
          const checkEmailResponse = await fetch("https://mas-host-least-disciplines.trycloudflare.com/backend/actions/checkEmail.php", {
            method: "POST",
            headers: {
        ...
        const purchaseResponse = await fetch("https://mas-host-least-disciplines.trycloudflare.com/backend/actions/completePurchase.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
