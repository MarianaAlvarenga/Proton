  ...
  const handleClick = async () => {
    if (NameButton === "Finalizar compra") {
      try {
        ...
        if (isRegistered) {
          const checkEmailResponse = await fetch("https://unless-scene-secrets-burst.trycloudflare.com/backend/actions/checkEmail.php", {
            method: "POST",
            headers: {
        ...
        const purchaseResponse = await fetch("https://unless-scene-secrets-burst.trycloudflare.com/backend/actions/completePurchase.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
