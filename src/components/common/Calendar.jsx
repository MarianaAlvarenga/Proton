...
export default function Calendar({
  ...
  const fetchDisponibilidades = (idPeluquero) => {
    ...
    setLoading(true);
    fetch(`https://before-discussion-picked-informational.trycloudflare.com/backend/actions/get_availabilities.php?id_peluquero=${idPeluquero}`)
      .then((res) => {
        if (!res.ok) {
  ...
  const handleEventClick = async (info) => {
    ...
    if (isSettingAvailability && userRole === 3) {
      ...
      try {
        ...
        const res = await fetch("https://before-discussion-picked-informational.trycloudflare.com/backend/actions/delete_availability.php", {
          method: "POST",
    ...
    if (!isSettingAvailability || (userRole === 3 && isAgendarTurno)) {
      ...
      try {
        const res = await fetch("https://before-discussion-picked-informational.trycloudflare.com/backend/actions/save_appointment.php", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            fecha: fecha_disponible,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch("https://before-discussion-picked-informational.trycloudflare.com/backend/actions/availability.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id_peluquero: id_peluquero,
