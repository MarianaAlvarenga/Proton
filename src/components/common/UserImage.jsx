...
const UserImage = ({ userId, onTempImageSelected }) => {
    ...
    const fetchUserImage = async () => {
        try {
            const response = await fetch(
                `https://explained-branches-post-lease.trycloudflare.com/backend/actions/getUserImage.php?userId=${userId}`,
                { credentials: "include" }
            );
            ...
            if (!data.img_url) {
                ...
                setSelectedImage(
                    `https://explained-branches-post-lease.trycloudflare.com/backend/uploads/${data.img_url}?t=${Date.now()}`
                );
            }
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            const response = await fetch(
                "https://explained-branches-post-lease.trycloudflare.com/backend/actions/uploadUserImage.php",
                {
                    method: "POST",
                    body: formData,
                    credentials: "include"
            ...
            if (fileName.includes("?")) {
                setSelectedImage(`https://explained-branches-post-lease.trycloudflare.com/backend/uploads/${fileName}`);
            } else {
                setSelectedImage(`https://explained-branches-post-lease.trycloudflare.com/backend/uploads/${fileName}?t=${Date.now()}`);
            }
